...
from image_synthesis.distributed.distributed import reduce_dict, all_gather
...
class Solver(object):
    def __init__(self, config, args, model, dataloader, logger):
        ...
        self.logger = logger
        self.scheduler_valid = config["solver"]["scheduler_valid"]
    ...
    def sample(self, batch, phase='train', step_type='iteration'):
        ...
        with torch.no_grad(): 
            if self.debug == False:
                if self.args.amp:
                    with autocast():
                        samples = model.sample(batch=batch, step=self.last_iter, phase=phase)
                else:
                    samples = model.sample(batch=batch, step=self.last_iter, phase=phase)
            else:
                samples = model.sample(batch=batch[0].cuda(), step=self.last_iter, phase=phase)
            ...
            if phase == "valid":
                save_dir = os.path.join(self.image_dir, phase)
                os.makedirs(save_dir, exist_ok=True)
                save_path = os.path.join(save_dir, 'e{:010d}_itr{:010d}_rank{}{}'.format(self.last_epoch, self.last_iter%self.dataloader['train_iterations'], get_rank(), suffix))
                ...
                self.logger.add_images(tag='{}/{}e_{}itr'.format(phase, self.last_epoch, self.last_iter%self.dataloader['train_iterations']), img_tensor=im, global_step=step, dataformats='NCHW')
                ...
                im_grid.save(save_path + '.jpg')
                self.logger.log_info('save to {}'.format(save_path+'.jpg'))
                ...
                if 'scheduler' in op_sc:
                    if op_sc['scheduler']['step_iteration'] > 0 and (self.last_iter + 1) % op_sc['scheduler']['step_iteration'] == 0:
                        if isinstance(op_sc['scheduler']['module'], STEP_WITH_LOSS_SCHEDULERS):
                            if not self.scheduler_valid:
                                op_sc['scheduler']['module'].step(output.get('loss'))
                            if self.scheduler_valid:
    ...
    def validate_epoch(self):
        ...
        if val:
            ...
            for itr, batch in enumerate(self.dataloader['validation_loader']):
                data_time = time.time() - itr_start
                step_start = time.time()
                batch['text']['token'] = batch['text']['token'].to(self.model.device).squeeze(1)
                sample = self.sample(batch, phase="valid")
                if self.args.distributed:
                    clip_text_embedding = self.model.module.condition_codec.encode_text(batch["text"]["token"])
                    clip_img_embedding = self.model.module.condition_codec.encode_image(self.model.module.clip_img_preprocess(torchvision.transforms.Resize(224)((sample/255))))
                else:
                    clip_text_embedding = self.model.condition_codec.encode_text(batch["text"]["token"])
                    clip_img_embedding = self.model.condition_codec.encode_image(self.model.clip_img_preprocess(torchvision.transforms.Resize(224)((sample/255))))
                cos_sim = cosine_metric(clip_text_embedding, clip_img_embedding).mean().item()
                t = all_gather(cos_sim)
                cos_sim = sum(t)/len(t)
            ...
            if self.logger is not None:
                ...
                #     for k in loss_dict:
                #         info += ' | {}: {:.4f}'.format(k, float(loss_dict[k]))
                self.logger.add_scalar(tag='val/clip_score', scalar_value=cos_sim, global_step=self.last_epoch)
                # self.logger.log_info(info)
            ...
            if self.scheduler_valid:
                for op_sc_n, op_sc in self.optimizer_and_scheduler.items():
                    if 'scheduler' in op_sc:
                        if op_sc['scheduler']['step_iteration'] > 0 and (self.last_iter + 1) % op_sc['scheduler']['step_iteration'] == 0:
                            if isinstance(op_sc['scheduler']['module'], STEP_WITH_LOSS_SCHEDULERS):
                                op_sc['scheduler']['module'].step(cos_sim)
                            else:
                                op_sc['scheduler']['module'].step()
