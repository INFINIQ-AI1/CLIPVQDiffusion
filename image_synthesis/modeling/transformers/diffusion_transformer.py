...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        if learnable_cf:
            ...
            # self.empty_text_embed = torch.nn.Parameter(torch.randn(size=(1, 512), requires_grad=True, dtype=torch.float64))
            self.empty_text_embed = torch.nn.Parameter(torch.randn(size=(1, 512), requires_grad=True, dtype=torch.float))
            # self.empty_text_embed = self.empty_text_embed/self.empty_text_embed.norm()
    ...
    def forward(
        ...
        #             # meaning of 2 -> if 2 False it is empty token.
        #             # if given text is empty text -> cond emb is empty, else not empty
        #             # then there should be some portion to be empty and some portion not empty? 
        ...
        if self.learnable_cf:
            cf_mask = prob_mask_like(cond_emb.shape[0], self.cf_drop_prob, cond_emb.device)
            cond_emb = torch.where(cf_mask.unsqueeze(1), (self.empty_text_embed/self.empty_text_embed.norm()).repeat(cond_emb.shape[0],1), cond_emb.type_as(self.empty_text_embed))
            
        # now we get cond_emb and sample_image
        if is_train == True:
            log_model_prob, loss = self._train_loss(sample_image, cond_emb)
            # divide by batch size & length (16 x 256)
