...
class AdaInsNormGivenEmb(nn.Module):
...
class AdaLayerNormCLIP(nn.Module):
    def __init__(self, n_embd):
        super().__init__()
    ...
    def forward(self, x, emb):
        ...
        #     emb = self.linear(self.silu(_emb)).unsqueeze(1)
        # else:
        emb = self.linear(self.silu(self.emb(emb))).unsqueeze(1)
        scale, shift = torch.chunk(emb, 2, dim=2)
        x = self.layernorm(x) * (1 + scale) + shift
        return x

class AdaInsNormCLIP(nn.Module):
    def __init__(self, n_embd):
        super().__init__()
        self.emb = nn.Linear(n_embd, n_embd)
        self.silu = nn.SiLU()
...
class Block(nn.Module):
    ...
    def __init__(self,
        ...
        if attn_type in ['self', 'selfcondition']:
            ...
            if attn_type == 'selfcondition':
                ...
                elif 'adainnormgivenemb' in class_type:
                    self.ln2 = AdaInsNormGivenEmb(n_embd)
                elif 'adalayerclip' in class_type:
                    self.ln2 = AdaLayerNormCLIP(n_embd)
                elif 'adainclip' in class_type:
                    self.ln2 = AdaInsNormCLIP(n_embd)
                else:
    ...
    def forward(self, x, encoder_output, timestep, mask=None):    
        if self.attn_type == "selfcross":
            ...
            x = x + a
            if self.class_type in ["adainnormgivenemb", "adalayerclip", "adainclip"]:
                x = x + self.mlp(self.ln2(x, encoder_output))
            else:
